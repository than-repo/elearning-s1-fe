...
import { createVnpayPaymentUrl } from "../api/payment-api";
...
export function VnpayCheckout({ course }: VnpayCheckoutProps) {
  return (
    <RoleProtectedRoute allowedRoles={["LEARNER"]} fallbackHref="/courses">
      <VnpayCheckoutContent course={course} />
    </RoleProtectedRoute>
  );
}

function VnpayCheckoutContent({ course }: VnpayCheckoutProps) {
    ...
    <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-start">
      <div className="rounded-lg border border-border bg-card p-5">
        <p className="text-sm font-semibold text-primary">
          {isFree ? "Free enrollment" : "VNPay secure payment"}
        </p>
        <h1 className="mt-3 max-w-2xl text-4xl font-semibold leading-tight">
          Confirm your enrollment
        </h1>
        <p className="mt-3 leading-7 text-muted-foreground">
          Review this course before continuing. Paid courses redirect to VNPay,
          and access is granted after the payment gateway confirms the payment.
        </p>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-semibold">
                  {isFree ? "Free enrollment" : "VNPay"}
                </p>
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                  {isFree
                    ? "No payment is required for this course."
                    : "You will be redirected to VNPay to complete payment securely."}
                </p>
              </div>
      ...
      <aside className="rounded-lg border border-border bg-card p-5 lg:sticky lg:top-16">
        ...
        <div className="mt-5 grid gap-3 border-y border-border py-4">
          ...
          <PaymentRow
            label="Provider"
            value={isFree ? "Free enrollment" : "VNPay"}
          />
          <PaymentRow label="Method" value={isFree ? "FREE" : "VNPAY"} />
        </div>
        ...
        <p className="mt-3 text-sm leading-6 text-muted-foreground">
          Enrollment is created only after payment confirmation succeeds.
        </p>
      </aside>
    </section>
  );
